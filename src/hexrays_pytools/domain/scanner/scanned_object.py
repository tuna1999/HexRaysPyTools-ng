"""``ScanObject`` factory hierarchy — typed expressions a scanner can match.

Ported from the original ``api.py:13-205`` (~190 LOC). Every ctree-traversing
component (visitors, scanners, actions) goes through ``ScanObject.create()``
to turn a raw ``cexpr_t`` (or ``ctree_item_t``) into a typed object whose
``is_target(cexpr)`` knows how to recognize it during traversal.

Two layers of types live in the project:

* ``ScanObject`` (no 'd') — what visitors **match** during traversal. Created
  here, from a cexpr.
* ``ScannedObject`` (with 'd') — what scanners **produce** when they find
  matches. Created in Phase A.6, wraps a ``ScanObject`` to carry the
  expression address + struct offset + an ``apply_type()`` callback.
...
``ScannedObject.create`` (Phase A.6) and ``is_legal_type`` checks depend on
their numeric values. Renumbering them would break saved netnode state.
"""
