# Task 0.8 Brief: tools/mock_ida.py + tests/conftest.py

## Files to Create

1. `D:\re_dev_projects\ida-plugins\HexRaysPyTools\tools\mock_ida.py`
2. `D:\re_dev_projects\ida-plugins\HexRaysPyTools\tests\conftest.py`
...
This task has no tests of its own. Verify by running the existing test suite:

```bash
cd D:\re_dev_projects\ida-plugins\HexRaysPyTools
PYTHONPATH=src pytest tests/pure/ -v
# Expected: all existing tests still pass
python -m mypy --strict tools/mock_ida.py
python -m ruff check tools/mock_ida.py tests/conftest.py
```
...
`D:\re_dev_projects\ida-plugins\HexRaysPyTools\docs\superpowers\task-reports\task-0.8-report.md`
